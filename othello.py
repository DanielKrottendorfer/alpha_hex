...
class OthelloNNet(nn.Module):
    ...
    def forward(self, x):
        ...
        s = x.view(-1, 1, self.board_x, self.board_x)                # batch_size x 1 x board_x x board_y
        ...
        s = F.dropout(F.relu(self.fc_bn1(self.fc1(s))), p=0.1, training=True)  # batch_size x 1024
        s = F.dropout(F.relu(self.fc_bn2(self.fc2(s))), p=0.1, training=True)  # batch_size x 512

        pi = self.fc3(s)                                                                         # batch_size x 1
        pi = pi.view(self.board_x,self.board_x)
